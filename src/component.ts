...
export function Component({ selector, ...options }: ComponentOptionsDecorated) {
    return (ctrl: IControllerConstructor) => {
        ...
        const selectorName = isAttrSelector
            ? getAttributeName(selector)
            : selector;
        defineMetadata(metadataKeys.name, kebabToCamel(selectorName), ctrl);
        defineMetadata(
            metadataKeys.declaration,
            isAttrSelector ? Declaration.Directive : Declaration.Component,
            ctrl
        );
...
export function registerComponent(
    module: IModule,
    component: IComponentController
) {
    const name = getMetadata(metadataKeys.name, component);
    const options = getMetadata(metadataKeys.options, component);
    const listeners: IHostListeners = getMetadata(
        metadataKeys.listeners,
        options.controller
    );
    const viewChildren: IViewChildren = getMetadata(
        metadataKeys.viewChildren,
        component
    );
    if (listeners || viewChildren) {
        options.controller = extendWithHostListenersAndChildren(
            options.controller,
            listeners,
            viewChildren
        );
...
/** @internal */
export function extendWithHostListenersAndChildren(
    //@ts-ignore
    ctrl: { new (...args: any[]) },
    ...
    class NewCtrl extends ctrl {
        constructor(private $element: JQLite, ...args: any[]) {
            super(...args);
        }
            ...
            properties.forEach((property) => {
                ...
                if (typeof child.selector !== 'string') {
                    ...
                    if (
                        ...
                        console.error(
                            `No valid selector was provided for ViewChild${
                                child.first ? '' : 'ren'
                            } decorator, it should be type or selector of component/directive`
                        );
                        return;
                    }
                ...
                const viewChildEls = Array.prototype.slice
                    .call(this.$element[0].querySelectorAll(selector))
                    .map((viewChild: Element) => {
                        ...
                        const componentName =
                            typeof child.selector === 'string'
                                ? viewChild.localName
                                : selector;
                        const el = angular.element(viewChild);
                        const $ctrl =
                            el && el.controller(kebabToCamel(componentName));
                        return child.read
                            ? new ElementRef(el)
                            : $ctrl || new ElementRef(el);
                    })
                    .filter((el) => !!el);

                if (viewChildEls.length) {
                    this[property] = child.first
                        ? viewChildEls[0]
                        : viewChildEls;
                } else {
...
function getHooksOnCtrlClass(ctrlClass: any): string[] {
    return Object.keys(ngLifecycleHooksMap).filter((hook: string) =>
        isFunction(ctrlClass[hook])
    );
}
