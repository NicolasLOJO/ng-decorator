...
export interface ModuleConfig {
    id?: string;
    declarations?: Array<
        IComponentController | Injectable<IDirectiveFactory> | PipeTransform
    >;
...
export function NgModule({
    ...
    return (Class: NgModule) => {
        ...
        const deps = imports
            .filter(
                (mod) =>
                    typeof mod === 'string' || typeof mod.module !== 'undefined'
            )
            .map((mod) =>
                typeof mod === 'string' ? mod : (mod.module as IModule).name
            );
        let moduleName = id;
        if (!moduleName) {
            console.warn(
                "You are not providing ngModule id, be careful this code won't work when uglified."
            );
            moduleName = (Class as any).name;
        }
        const module = angular.module(moduleName as string, deps);

        // components, directives and filters registration
        declarations.forEach((declaration: any) => {
            const declarationType = getMetadata(
