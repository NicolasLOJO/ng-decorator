...
import {
    extendWithHostListenersAndChildren,
    replaceLifecycleHooks,
} from './component';
...
export function Directive({ selector, ...options }: DirectiveOptionsDecorated) {
    return (ctrl: DirectiveControllerConstructor) => {
        ...
        const selectorName = isAttributeSelector(selector)
...
export function registerDirective(
    module: IModule,
    ctrl: DirectiveControllerConstructor
) {
    let directiveFunc;
    ...
    options.controller =
        listeners || viewChildren
            ? extendWithHostListenersAndChildren(ctrl, listeners, viewChildren)
            : ctrl;
