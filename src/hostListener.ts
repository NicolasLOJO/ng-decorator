...
export function HostListener(eventName?: string, args?: string[]) {
    return (
        ...
        if (typeof listener !== 'function') {
            throw new Error(
                `@HostListener decorator can only be applied to methods not: ${typeof listener}`
            );
        }
         ...
         */
        const listeners: IHostListeners =
            getMetadata(metadataKeys.listeners, targetConstructor) || {};
