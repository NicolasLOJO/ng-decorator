...
describe('NgModule', () => {
    ...
    describe('has run and config methods', () => {
        it('module should have run and config blocks', () => {
            ...
            expect(angular.module(moduleName)['_configBlocks'][0].length).toBe(
                3
            );

            expect(angular.module(moduleName)['_runBlocks'][0]).toBe(
                NgModuleClass.run
            );
            expect(angular.module(moduleName)['_configBlocks'][0][2][0]).toBe(
                NgModuleClass.config
            );
        });
    });
    ...
    describe('imports', () => {
        it('should define required module as dependency', () => {
            ...
            const importedModule = registerNgModule(
                importedModuleName,
                [],
                [],
                []
            ...
            expect(angular.module(moduleName).requires).toEqual([
                importedModuleName,
            ]);
        });
    });
        ...
        describe('@Directive:', () => {
            ...
            describe('@Input and @Output', () => {
                it('assigns properties to @Directive bindToController bindings', () => {
                    ...
                    registerNgModule(moduleName, [], [myDirective]);
                    const invokeQueue =
                        angular.module(moduleName)['_invokeQueue'];
            ...
            describe('@HostListener', () => {
                it('injects $element and adds $postLink and $onDestroy lifecycle hooks', () => {
                    ...
                    const invokeQueue =
                        angular.module(moduleName)['_invokeQueue'];
                    const ctrlProto =
                        invokeQueue[0][2][1]().controller.prototype;
                    const inject = ctrlProto['constructor']['$inject'];
    ...
    describe('providers', () => {
        describe('provided as array of classes', () => {
            it('registers provider using class type', () => {
                ...
                expect(
                    angular.module(moduleName)['_invokeQueue'].length
                ).toEqual(providers.length);
                angular
                    .module(moduleName)
                    ['_invokeQueue'].forEach((value: any, index: number) => {
        ...
        describe('provided using useClass syntax', () => {
            it('registers provider using provide token', () => {
                const providers = [
                    { provide: 'useClassTestService', useClass: TestService },
                ];
                registerNgModule(moduleName, [], [], providers);
        ...
        describe('useFactory', () => {
            it('registers provider using string token', () => {
                const providers = [
                    {
                        provide: 'useFactoryTestService',
                        useFactory: (...args) => new TestService(args),
                    },
                ];
                registerNgModule(moduleName, [], [], providers);
        ...
        describe('useValue', () => {
            it('registers provider using string token', () => {
                const providers = [
                    {
                        provide: 'useValueTestService',
                        useValue: (...args) => new TestService(args),
                    },
                ];
                registerNgModule(moduleName, [], [], providers);
    ...
    describe('@Pipe', () => {
        ...
        describe('without injection', () => {
            ...
            it('registers as filter', () => {
                registerNgModule(moduleName, [], [FormatDateTimeFilter]);
                const invokeQueue = angular.module(moduleName)['_invokeQueue'];
        ...
        describe('with injection', () => {
            ...
            it('registers as filter', () => {
                ...
                expect(invokeQueue[0][1]).toEqual('register');
                expect(invokeQueue[0][2][0]).toEqual(name);
                expect(invokeQueue[0][2][1].$inject).toEqual([
                    '$injector',
                    '$timeout',
                ]);
            });
